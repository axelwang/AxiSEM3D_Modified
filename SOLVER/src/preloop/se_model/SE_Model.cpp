...
computeDt(double courant, const ABC &abc, eigen::DCol2 &sz) const {
    ...
    //vvvvvvvvvvvvvvvvvvv Added by Axel
    /* Create a file to pass in to computeDt for output */
    std::ofstream outData;
    outData.open("regularGLL.txt", std::ios::app);  // append mode



    for (const Quad &quad: mQuads) {

        // Get a list of all element tags
        // std::ofstream gtags;
        // gtags.open("allElementTags.txt", std::ios::app);  
        int elemTag = quad.getGlobalTag();
        // gtags << elemTag << std::endl;
        
        /*   Select which discontinuity for plotting   */
        double mohoDepth = 30e3;
        double lvzTopDepth = 80e3;
        double lvzBotDepth = 220e3; 
        double mohoRadius = 6371e3-mohoDepth;   
        double lvzTopRadius = 6371e3-lvzTopDepth;
        double lvzBotRadius = 6371e3-lvzBotDepth;  
        double eps = 0.1e3;
        eigen::DMat2N szQuad = quad.getPointSZ();
        std::ofstream se;
        se.open("startingElement.txt", std::ios::app); // append mode
        //-------------------- Moho
        if (szQuad(0,1) == 0 && szQuad(1,0) == mohoRadius){
            se << "The starting element tag for"
                        " Moho at radius:" << szQuad(1,0) << "m is : " << quad.getGlobalTag()<< std::endl;
                                                 
        }
        else if (szQuad(0,1) == 0 && abs(szQuad(1,0)-mohoRadius) <= eps) {
            se.precision(20);
            se<< "Precise value for Moho not found! The closest value is " << szQuad(1,0) << "m."  
                         "The starting element tag for Moho is:" << quad.getGlobalTag() << std::endl;
        }
        //-------------------- LVZ Top
        if (szQuad(0,1) == 0 && szQuad(1,0) == lvzTopRadius){
            se << "The starting element tag for"
                        " LVZ Top at radius:" << szQuad(1,0) << "m is : " << quad.getGlobalTag()<< std::endl;
                                                 
        }
        else if (szQuad(0,1) == 0 && abs(szQuad(1,0)-lvzTopRadius) <= eps) {
            se.precision(20);
            se<< "Precise value for LVZ Top not found! The closest value is " << szQuad(1,0) << "m."  
                         "The starting element tag for LVZ Top is:" << quad.getGlobalTag() << std::endl;
        }

        //------------------- LVZ Bot
        if (szQuad(0,1) == 0 && szQuad(1,0) == lvzBotRadius){
            se << "The starting element tag for"
                        " LVZ Bottom at radius:" << szQuad(1,0) << "m is : " << quad.getGlobalTag()<< std::endl;
                                                 
        }
        else if (szQuad(0,1) == 0 && abs(szQuad(1,0)-lvzBotRadius) <= eps) {
            se.precision(20);
            se<< "Precise value for LVZ Bottom not found! The closest value is " << szQuad(1,0) << "m."  
                         "The starting element tag for LVZ Bottom is:" << quad.getGlobalTag() << std::endl;
        }
        ...
        double dt = quad.computeDt(courant, abc, outData,elemTag);
        if (dtMin > dt) {
            dtMin = dt;
