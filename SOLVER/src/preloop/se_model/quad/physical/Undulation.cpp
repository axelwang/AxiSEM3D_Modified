...
void Undulation::finished3D(const Quad &myQuad) {   
    ...
    //vvvvvvvvvvvvvvvvvvvvvvvvvvvvv Added by Axel 
    std::ofstream ff;
    ff.open("fCoeffs.txt", std::ios::app);  // append mode
    std::ofstream qn;
    qn.open("undulatedElementTags.txt", std::ios::app);  // append mode

    if (sDeltaZ_Fourier.size()<=1) {
        std::cout<<"Fourier series NOT PROPERLY constructed" << std::endl;
    }

    for (int i = 0; i < sDeltaZ_Fourier.size(); i++)
    {
        //ff << "This is element: " << myQuad.getGlobalTag() << std::endl;
        ff << sDeltaZ_Fourier[i][0] << std::endl;
    }
    qn<< myQuad.getGlobalTag() << std::endl;
