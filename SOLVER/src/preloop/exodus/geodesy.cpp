...
namespace geodesy {
    ...
    void setup(ExodusMesh &exMesh) {
        ...
        // Q matrix from source-centered to geographic
        // vvvvvvvvvvvvvvvvvvvvvvvv Added by Axel
        // changed below to false for correct for elipticity.
        // this should be consistent with input file so hardcoded
        // true maybe a bug
        // ^^^^^^^^^^^^^^^^^^^^^^^^Added by Axel
        eigen::DRow3 srctpr = llr2tpr(internal::iLatLonRadiusAxisZ, false);
