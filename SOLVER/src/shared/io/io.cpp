...
namespace io {
    ...
    void verifyDirectories(int argc, char *argv[], std::string &warning) {
        ...
        // Changed by Axel: find environment variable for local scratch directory 
        const char *envVar = std::getenv("L_SCRATCH_JOB");
        const std::string mpi_rank = mpi::strRank();
        if (envVar == nullptr) {
           std::cout<<"The environment variable does not exist"<<std::endl;
        }
        /* else {
           std::cout<<"Outputting to Local Scratch:" << envVar<<"/task"<<mpi_rank<<std::endl;
        } 
        */

        gOutputDirectory = envVar;
        gOutputDirectory +="/task"+mpi_rank;
        ...
        if (mpi::root()) {
            ...
            // create output directories that only output on root rank
       ...
       */
        // create output folders outputting on all ranks
    ...
    std::string welcome() {
        ...
        "[This version modified by Axel Wang.                         ]"
        "[Outputting to Local Scratch.                                ]"
        "{~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~}\n\n";
